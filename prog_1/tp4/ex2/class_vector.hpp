...
public:
    ...
    friend Vector operator+(const Vector<Ndim,T>& v1,const Vector<Ndim,T>& v2){
        Vector temp;
        temp.zero();
        for (int ii = 0; ii < Ndim; ii++)
            temp[ii] = v1[ii] + v2[ii];
...
// T est un tableau statique et doit donc être initialise pour definir un Vector de Vectors
template<int Ndim, typename T>
Vector<Ndim, T>::Vector() {
    for (int i = 0; i < Ndim; i++) {
        coord[i] = T();  // Assurer que chaque élément est bien initialisé
    }
}
