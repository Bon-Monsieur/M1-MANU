...
def SFS_fixed_point_method(Nx, Ny, fig="parabola",epsilon=1e-4,maxiter=2000):
    ...
    for k in range(maxiter+1):
        ...
        Un[1:-1, 1:-1] = 1/2*(np.maximum(np.minimum(Un[0:-2,1:-1],Un[2:,1:-1]),np.minimum(Un[1:-1,0:-2],Un[1:-1, 2:]))  
                              + np.sqrt(np.maximum(np.minimum(Un[0:-2,1:-1],Un[2:,1:-1]),np.minimum(Un[1:-1,0:-2],Un[1:-1, 2:]))**2 
                                        + n(X[1:-1],Y[1:-1])))
